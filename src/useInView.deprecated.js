...
/**
 * useAuthGate — returns a function that runs the callback only if the user is
 * authenticated; otherwise it calls onUnauth (e.g., to open the auth modal).
 *
 * @example
 *   const guard = useAuthGate(() => setShowAuthModal(true));
 *   <button onClick={guard(() => handleAddLead())}>Add Lead</button>
 */
export function useAuthGate(onUnauth) {
  const { isAuthenticated } = require("./AuthContext").useAuth();
  return (fn) => (...args) => {
